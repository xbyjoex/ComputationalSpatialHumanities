...
async def election_summary(election_id: str, _user: CurrentUser) -> ORJSONResponse:
    ...
    async with get_conn() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                """
                SELECT party, party_index, erststimmen, zweitstimmen,
                       ROUND(zweitstimmen::numeric * 100 / NULLIF(gueltige_zweit, 0), 2) AS anteil_pct,
                       wahlberechtigte, waehler, briefwaehler, gueltige_zweit
                FROM core.election_results
                WHERE election_id = %s AND level = 'stadt'
                ORDER BY zweitstimmen DESC NULLS LAST
                """,
                (election_id,),
            )
